...
import { DeviceType, Client } from "@/types";
  ...
  devtools(
    (set, get) => ({
      ...
      submitForm: async () => {
        ...
        if (
          !clientToUse &&
          formData.newClient.firstName &&
          formData.newClient.lastName &&
          formData.newClient.phone
        ) {
          try {
            const newUserResponse = await api.post("/users/", {
              username:
                formData.newClient.email ||
                `${formData.newClient.firstName}.${formData.newClient.lastName}.${Date.now()}`,
        ...
        try {
          ...
          const repairData = {
            ...
            description:
              formData.issueDescription || "Réparation créée via formulaire",
          ...
          const repairResponse = await api.post(
            "/repairs/repairs/",
            repairData,
          );
