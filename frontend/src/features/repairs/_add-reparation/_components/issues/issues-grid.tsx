...
const IssuesGrid: React.FC<Props> = ({
  ...
  if (isLoading) {
    return (
      <div className={cn("flex items-center justify-center py-8", className)} {...rest}>
  ...
  return (
    <div className={cn("grid grid-cols-1 md:grid-cols-2 gap-4", className)} {...rest}>
      {issues.map((issue) => (
        <IssueCard
          key={issue.id}
          issue={issue}
          isSelected={selectedIssues.some((i) => i.issueId === String(issue.id))}
