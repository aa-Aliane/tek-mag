          ...
          >
            <div className="flex items-center justify-between">
              <div className="font-medium">
                {tier.qualityTier === "standard" && "Standard"}
                {tier.qualityTier === "premium" && "Premium"}
                {tier.qualityTier === "original" && "Original"}
                {tier.qualityTier === "refurbished" && "Reconditionné"}
              </div>
              {selectedTierId === tier.id && (
            ...
            <div className="text-sm text-muted-foreground mt-2">
              {tier.warrantyDays} jours garantie
            </div>
            <div className="mt-auto pt-2">
              <div className="text-xs">
                {tier.availabilityStatus === "in_stock" && (
                  <span className="text-green-600 bg-green-100 px-2 py-1 rounded-full">
                    En stock
                  </span>
                )}
                {tier.availabilityStatus === "low_stock" && (
                  <span className="text-yellow-600 bg-yellow-100 px-2 py-1 rounded-full">
                    Stock limité
                  </span>
                )}
                {tier.availabilityStatus === "out_of_stock" && (
                  <span className="text-red-600 bg-red-100 px-2 py-1 rounded-full">
                    Rupture de stock
                  </span>
                )}
                {tier.availabilityStatus === "discontinued" && (
