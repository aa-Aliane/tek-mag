...
class IssueSerializer(serializers.ModelSerializer):
    device_types = serializers.PrimaryKeyRelatedField(
        queryset=DeviceType.objects.all(), many=True
    )
    # Return IDs instead of full objects for performance
    associated_part = serializers.PrimaryKeyRelatedField(read_only=True)
    compatible_parts = serializers.PrimaryKeyRelatedField(many=True, read_only=True)
